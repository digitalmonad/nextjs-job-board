...
import { approveJobAction } from "../_actions/approve-job-action";
import { deleteJobAction } from "../_actions/delete-job-action";
...
function ApproveSubmissionButton({ jobId }: AdminButtonProps) {
  const [formState, formAction] = useFormState(approveJobAction, undefined);
...
function DeleteJobButton({ jobId }: AdminButtonProps) {
  const [formState, formAction] = useFormState(deleteJobAction, undefined);
